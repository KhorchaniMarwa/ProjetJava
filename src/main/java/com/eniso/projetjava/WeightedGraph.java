...
class WeightedGraph extends Graph {
   ...
   void addEdge(int vertex1, int vertex2) {
        ...
        weights.put(new int[]{vertex1, vertex2}, 1); 
    }
    ...
    void addEdge(int vertex1, int vertex2, int weight) {
        ...
        addVertex(vertex2);
        edges.add(new int[]{vertex1, vertex2});
        weights.put(new int[]{vertex1, vertex2}, weight);
    }

    void printGraph() {
        System.out.println("Weighted Graph:");
        for (Map.Entry<int[], Integer> entry : weights.entrySet()) {
            int[] edge = entry.getKey();
            int weight = entry.getValue();
            System.out.println(edge[0] + " --(" + weight + ")-> " + edge[1]);
        }
    }
